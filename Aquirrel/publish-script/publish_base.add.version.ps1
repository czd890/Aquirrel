...
function SetVer ($ver, $level, $value) {
    write-Host "set ver:"$ver",level:"$level+",value:"$value
    $verArr = $ver.Split('.')
...
try {
    ...
    if (![string]::IsNullOrEmpty($releasenote)) {
        write-Host "release notes: "$projectXml.Project.PropertyGroup[0].PackageReleaseNotes" -->" $releasenote
        $projectXml.Project.PropertyGroup[0].PackageReleaseNotes = $releasenote
    }
    $currVer = $projectXml.Project.PropertyGroup[0].VersionPrefix
    ...
    write-Host "version: "$projectXml.Project.PropertyGroup[0].VersionPrefix" -->"$currVer
    $projectXml.Project.PropertyGroup[0].VersionPrefix = $currVer
}catch {
    Write-Host $_
    Set-Location $orgDir
    return
